...
std::string render_white(std::string str){
	std::stringstream ss;
	for(char c : str)
		if(c == ' ')
			ss << "·";
...
std::pair<std::string, std::string> render_up_down(std::string a_text, std::string b_text) { //@davidsena
    ...
    auto replace = [](std::string s, char a, std::string b) {
        std::stringstream ss;
        for (char &c : s)
            if (c == a)
                ss << b;
    ...
    std::string char_error = "≠ ";
    std::string char_equal = "│ ";

    auto a_lines = splitlines(a_text);
    auto b_lines = splitlines(b_text);

    char mark = '\a';

    auto a_render = splitlines(replace(a_text, '\n', {mark, '\n'}));
    auto b_render = splitlines(replace(b_text, '\n', {mark, '\n'}));

    std::vector<std::string> a_output;
    std::vector<std::string> b_output;

    int a_size = a_lines.size();
    int b_size = b_lines.size();
    int first_failure = -1;
    int max_size = std::max(a_size, b_size);
    auto geti = [](std::vector<std::string> &v, int i) -> std::string {
        return (i < 0 || i >= (int) v.size()) ? "" : v[i];
    };

    for (int i = 0; i < max_size; i++) {
        if (i >= a_size || i >= b_size || a_render[i] != b_render[i]) {
            if (first_failure == -1)
                first_failure = i;
            a_output.push_back(char_error + geti(a_lines, i));
            b_output.push_back(char_error + geti(b_lines, i));
        } else {
            a_output.push_back(char_equal + geti(a_lines, i));
            b_output.push_back(char_equal + geti(b_lines, i));
        }
    }
    std::ostringstream a_ss;
    for (std::string line : a_output)
        a_ss << line << "\n";

    std::ostringstream b_ss;
    for (std::string line : b_output)
        b_ss << line << "\n";

    if (first_failure != -1) {    
        auto a_first = geti(a_render, first_failure);
        auto b_first = geti(b_render, first_failure);
        int greater = std::max(a_first.size(), b_first.size());

        b_ss << "\n\n--------------------------------------\n";
        b_ss << "First line mismatch showing withspaces\n\n";
        b_ss << std::left;
        auto a_proc = render_white(replace(a_first, mark, "↵"));
        auto b_proc = render_white(replace(b_first, mark, "↵"));
        b_ss << a_proc << std::string(greater - a_first.size(), ' ') << " (expected)\n";
        b_ss << b_proc << std::string(greater - b_first.size(), ' ') << " (received)\n";
    }
    return {a_ss.str(), b_ss.str()};
}
...
string TestCase::getComment() {
	...
	if (!correctOutput) {
		string token;
		stringstream ss(this->caseDescription);
		bool hide_all = false;
		bool hide_expected_only = false;
		for(int i = 0; i < 2; i++){
			if(ss >> token){
				hide_all = hide_all || (token.find("__") == 0);
				hide_expected_only = hide_expected_only || (token.find("_") == 0);
			}
		}
		...
		ret += " --- Input ---\n";
		if(hide_all) {
			ret += Tools::caseFormat("Hiding Case");
		} else {
			ret += Tools::caseFormat(input);
		}

		std::string received = programOutputBefore + programOutputAfter;
		std::string expected = ""; 
		if (output.size() > 0) {
			expected = output[0]->studentOutputExpected();
		}

		auto answer = render_up_down(expected, received);
		expected = answer.first;
		received = answer.second;


		if(output.size()>0){
			ret += "\n --- Expected output ("+output[0]->type()+")---\n";
			if(hide_expected_only) {
				ret += Tools::caseFormat("Hiding Case");
			} else {
				ret += Tools::caseFormat(expected);
			}
		}
 ...
 * David Sena Oliveira
 */
std::string tio_2_vpl(std::string text) {
    std::stringstream ss(text), input, output, result;
    std::string line, case_name, open_line, middle_line;
    int grade_reduction { 0 };

    auto flush = [&]() {
        result << open_line << input.str() << middle_line << output.str();
    };
    auto clean = [&]() {
        case_name = "";
        grade_reduction = 0;
    ...
    auto process_case_and_gr = [&grade_reduction, &case_name](std::string line) {
        std::stringstream line_ss(line);
        std::string token;
        line_ss >> token; //>>>>>>>>
        while (line_ss >> token) {
            size_t grpos = token.find('%');
            if (grpos != std::string::npos) { //grade reduction token
                if (token.size() == 1) {
                    grade_reduction = 0;
                } else {
                    token[grpos] = ' ';
                    std::istringstream(token) >> grade_reduction;
                }
            } else {
                case_name += " " + token;
            }
    ...
    while(std::getline(ss, line)) {
        ...
        if (line.substr(0, 8) == ">>>>>>>>") {
            ...
            clean();
            open_line = line;
            process_case_and_gr(line);
            open = true;
            middle = false;
        } else if (open && line.substr(0, 8) == "========") {
            middle_line = line;
            middle = true;
        } else if (middle && line.substr(0, 8) == "<<<<<<<<") {
            middle = false;
            open = false;
            case_name = case_name.size() > 0 ? case_name.substr(1) : case_name;
            result << "case=" + case_name + "\ninput=" << input.str() << "output=\"" << output.str() << "\"\n";
            result << (grade_reduction != 0 ? "grade reduction=" + std::to_string(grade_reduction) + "%\n" : "\n"); 
            clean();
        } else if (middle) {
            output << line;
        } else if (open) {
...
void Evaluation::loadTestCases(string fname) {
	...
	bool inCase = false;
	vector<string> lines = Tools::splitLines(tio_2_vpl(Tools::readFile(fname)));
    remove(fname.c_str());
	string inputEnd = "";
	string outputEnd = "";
	Case caso;
